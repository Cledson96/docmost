...
- Os módulos de rota de workspace e páginas compartilhadas mantêm eager o comportamento de query, erro e metadados. A rota de workspace carrega `FullEditor` junto com seu próprio módulo; o histórico fica em uma fronteira `Suspense` independente e não pode ocultar o documento. Dentro de `FullEditor`, `PageEditor` (Yjs/Hocuspocus) só é carregado para edição ou comentários inline, que exigem seleções relativas Yjs; leitores sem essas permissões usam `ReadonlyPageEditor` sem conexão de colaboração. A página compartilhada continua adiando `ReadonlyPageEditor`, e falhas de chunks lazy se recuperam com ações de recarregamento.
