...
- Os módulos de rota de workspace e páginas compartilhadas mantêm eager o comportamento de query, erro e metadados. A rota de workspace carrega `FullEditor` junto com seu próprio módulo; o histórico fica em uma fronteira `Suspense` independente e não pode ocultar o documento. Dentro de `FullEditor`, `CollaborativePageEditor` baixa `PageEditor` (Yjs/Hocuspocus) por um loader controlado por estado e mantém o documento em `ReadonlyPageEditor` até o módulo montar; assim, um lazy chunk pendente não deixa a página em branco. Leitores sem colaboração usam `ReadonlyPageEditor` diretamente. `SharedPage` importa o leitor junto com seu módulo de rota para que uma consulta pública já resolvida nunca fique suspensa em uma segunda importação; falhas reais de renderização continuam no error boundary com ação de recarregar.
