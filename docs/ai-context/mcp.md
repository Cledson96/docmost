...
- `edit_page_blocks` aplica até 50 operações atômicas ao documento Yjs vivo. Exige `pageId`, a `expectedRevision` de `get_page` e operações endereçadas por ID; o conteúdo é Agent Markdown convertido no servidor. Ele valida edição da página e leitura de páginas referenciadas, rejeita campos desconhecidos ou `resolved` enviado pelo cliente e repassa o usuário autenticado ao handler. Falhas retornam `isError: true` com código estável.
