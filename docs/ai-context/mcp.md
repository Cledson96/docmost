...
- O formato padrão de conteúdo de página é Markdown; `get_page` também aceita HTML ou JSON ProseMirror. `create_page` e `update_page` recebem Agent Markdown, convertem-no com `agentMarkdownToProsemirror` e `tiptapExtensions`, e chamam `PageService` como `format: 'json'`; Markdown comum continua aceito e `update_page` preserva `append` (padrão), `prepend` ou `replace`.
- `edit_page_blocks` aplica até 50 operações atômicas ao documento Yjs vivo. Exige `pageId`, a `expectedRevision` de `get_page` e operações endereçadas por ID; cada conteúdo Agent Markdown é limitado a 1.000.000 caracteres e convertido no servidor. Ele valida edição da página e leitura de páginas referenciadas, rejeita campos desconhecidos ou `resolved` enviado pelo cliente e repassa o usuário autenticado ao handler. Falhas retornam `isError: true` com código estável.
