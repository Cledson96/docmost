...
- Socket.IO autentica pelo cookie e exige uma `user_session` ativa que corresponda ao usuário e workspace do JWT antes de colocar conexões em salas de usuário, workspace e spaces autorizados. `WsService` refaz filtragem por usuário ao transmitir mudanças de árvore de páginas restritas e desconecta sockets pelo `sessionId` quando a sessão é revogada ou removida por alteração de senha.
