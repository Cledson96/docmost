...
- `editPageBlocks` chama `applyRichContentOperations` em `collaboration/rich-content/block-operations.ts`: abre `page.<pageId>` com o contexto do usuário, preflighta cada operação em um Y.Doc isolado, compara `expectedRevision` e só então aplica uma transação ao documento vivo, devolvendo a revisão resultante. Falhas tipadas não deixam alterações parciais. Ele não grava páginas nem cria histórico diretamente; a persistência Yjs preserva atribuição e efeitos posteriores. Cada inserção, movimento e substituição valida a sequência resultante no container contra o schema ProseMirror, inclusive no fragmento raiz. Inserções usam `prosemirrorNodeToYElement`, e mutações pontuais preservam os objetos Yjs dos irmãos não tocados.
