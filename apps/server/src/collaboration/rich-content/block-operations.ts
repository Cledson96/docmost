...
export type BlockOperation =
  | {
      type: 'insertBefore' | 'insertAfter' | 'insertIn';
      target: string;
      content: JSONContent;
    }
...
export type ApplyBlockOperationsInput = {
...
export class BlockOperationError extends Error {
  constructor(
    public readonly code: BlockOperationErrorCode,
    message: string,
  ) {
    super(message);
  }
...
const capabilities = new Map(
  richContentCapabilities.map((item) => [item.name, item]),
);
const schema = getSchema(tiptapExtensions);

export function applyRichContentOperations(
  doc: Y.Doc,
  input: ApplyBlockOperationsInput,
): void {
  if (
    input.expectedRevision &&
    input.expectedRevision !== revisionForDocument(doc)
  ) {
    throw new BlockOperationError(
      'STALE_REVISION',
      'The page revision is stale',
    );
  }
  ...
  for (const operation of input.operations) {
    for (const locator of [
      operation.target,
      ...(operation.type === 'move' ? [operation.destination] : []),
    ]) {
      const match = /^legacy:([^:]+):/.exec(locator);
      if (match && match[1] !== revision)
        throw new BlockOperationError(
          'STALE_REVISION',
          'The legacy block locator is stale',
        );
    }
  }
...
function applyAll(doc: Y.Doc, operations: BlockOperation[]) {
  doc.transact(() =>
    operations.forEach((operation) => applyOne(doc, operation)),
  );
}
...
function applyOne(doc: Y.Doc, operation: BlockOperation) {
  ...
  if (!target)
    throw new BlockOperationError(
      'BLOCK_NOT_FOUND',
      `Block '${operation.target}' was not found`,
    );
  if (operation.type === 'delete') return target.parent.delete(target.index, 1);
  if (operation.type === 'update') {
    if (!(target.node instanceof Y.XmlElement))
      throw invalid('Only element blocks can be updated');
    for (const [key, value] of Object.entries(operation.attrs))
      target.node.setAttribute(key, value as any);
    return;
  }
  if (operation.type === 'replaceRange') {
    if (!(target.node instanceof Y.XmlElement))
      throw invalid('Only element blocks can contain a range');
    ...
    if (
      ...
      operation.to > target.node.length
    )
      throw invalid('Invalid replacement range');
    ...
    if (
      !canPlace(
        ...
        operation.from,
        operation.to - operation.from,
        operation.content.map(schemaNodeForContent),
      )
    )
      throw invalid('Only element blocks can contain a range');
  ...
  if (operation.type === 'move') {
    ...
    if (!destination)
      throw new BlockOperationError(
        'BLOCK_NOT_FOUND',
        `Block '${operation.destination}' was not found`,
      );
    ...
    if (position === 'in') {
      if (
        !(currentDestination.node instanceof Y.XmlElement) ||
        !canPlace(currentDestination.node, currentDestination.node.length, 0, [
          schemaNode,
        ])
      )
        throw invalid('Destination cannot contain blocks');
      currentDestination.node.insert(currentDestination.node.length, [node]);
    } else {
      const shifted =
        sameParent && target.index < destinationIndex
          ? destinationIndex - 1
          : destinationIndex;
      const index = shifted + (position === 'after' ? 1 : 0);
      if (!canPlace(currentDestination.parent, index, 0, [schemaNode]))
        throw invalid('Destination cannot contain blocks');
  ...
  if (operation.type === 'insertIn') {
    if (
      !(target.node instanceof Y.XmlElement) ||
      !canPlace(target.node, target.node.length, 0, [schemaNode])
    )
      throw invalid('Target cannot contain blocks');
    target.node.insert(target.node.length, [node]);
  } else {
    const index = target.index + (operation.type === 'insertAfter' ? 1 : 0);
    if (!canPlace(target.parent, index, 0, [schemaNode]))
      throw invalid('Target parent cannot contain blocks');
...
function validateOperation(doc: Y.Doc, operation: BlockOperation) {
  ...
  for (const id of collectIds(doc.getXmlFragment('default'))) {
    if (ids.has(id))
      throw new BlockOperationError(
        'DUPLICATE_BLOCK_ID',
        `Duplicate existing block id '${id}'`,
      );
    ids.add(id);
  }
  if (
  ...
  if (operation.type === 'update') {
    ...
    for (const [name, value] of Object.entries(operation.attrs)) {
      ...
      if (!attr || (name === 'id' && typeof value !== 'string'))
        throw new BlockOperationError(
          'INVALID_BLOCK_SCHEMA',
    ...
    if (typeof operation.attrs.id === 'string') {
      ...
      if (ids.has(operation.attrs.id))
        throw new BlockOperationError(
          'DUPLICATE_BLOCK_ID',
          `Duplicate block id '${operation.attrs.id}'`,
        );
    }
  }
}
...
function validateNode(content: JSONContent, ids: Set<string>) {
  if (content.type === 'text') {
    ...
    for (const mark of content.marks ?? []) {
      ...
      if (!capability || capability.category !== 'mark')
        throw new BlockOperationError(
          'INVALID_BLOCK_SCHEMA',
          `Unsupported mark '${mark.type}'`,
        );
    }
    return;
  }
  const capability = content.type ? capabilities.get(content.type) : undefined;
  if (!capability || content.type === 'doc')
    throw new BlockOperationError(
      'INVALID_BLOCK_SCHEMA',
      `Unsupported block type '${content.type ?? ''}'`,
    );
  const attributes = content.attrs ?? {};
  for (const attr of capability.attributes) {
    if (
      attr.required &&
      (attributes[attr.name] === undefined || attributes[attr.name] === null)
    )
      throw new BlockOperationError(
        'INVALID_BLOCK_SCHEMA',
  ...
  try {
    ...
    throw new BlockOperationError(
      'INVALID_BLOCK_SCHEMA',
      `Invalid content for '${content.type}'`,
    );
  }
}
  ...
  return parent
    ...
    .flatMap((node) =>
      node instanceof Y.XmlElement
        ? [node.getAttribute('id'), ...collectIds(node)].filter(
...
function cloneYNode(node: Y.XmlElement | Y.XmlText): Y.XmlElement | Y.XmlText {
  ...
  for (const [key, value] of Object.entries(node.getAttributes()))
    copy.setAttribute(key, value as any);
  copy.insert(
    0,
    node
      .toArray()
      .map((child) => cloneYNode(child as Y.XmlElement | Y.XmlText)),
  );
  return copy;
}
...
function canPlace(
  ...
  try {
    const children = parent
      .toArray()
      .map((node) => schemaNodeForYNode(node as Y.XmlElement | Y.XmlText));
...
function locate(
  ...
  for (const [index, node] of parent.toArray().entries()) {
    ...
    if (
      node instanceof Y.XmlElement &&
      (node.getAttribute('id') === locator ||
        (legacy?.[2] === currentPath && !node.getAttribute('id')))
    )
...
function invalid(message: string) {
  return new BlockOperationError('INVALID_OPERATION', message);
}
