...
describe('applyBlockOperations', () => {
  it('inserts after a top-level block without replacing untouched sibling identity', () => {
    const doc = documentWith([
      { type: 'paragraph', attrs: { id: 'one' } },
      { type: 'paragraph', attrs: { id: 'two' } },
    ]);
    const fragment = doc.getXmlFragment('default');
    const untouched = fragment.get(1);

    applyBlockOperations(doc, {
      operations: [
        {
          type: 'insertAfter',
          target: 'one',
          content: { type: 'paragraph', attrs: { id: 'new' } },
        },
      ],
    });

    expect(
      fragment.toArray().map((node: any) => node.getAttribute('id')),
    ).toEqual(['one', 'new', 'two']);
  ...
  it('edits nested content in place and moves it without replacing untouched siblings', () => {
    const doc = documentWith([
      {
        ...
        attrs: { id: 'parent' },
        content: [
          { type: 'paragraph', attrs: { id: 'nested' } },
          { type: 'paragraph', attrs: { id: 'stay' } },
        ],
      },
    ]);
    ...
    applyBlockOperations(doc, {
      operations: [
        ...
        {
          type: 'replaceRange',
          target: `legacy:${revisionForDocument(doc)}:0`,
          from: 0,
          to: 1,
          content: [{ type: 'paragraph', attrs: { id: 'replacement' } }],
        },
        {
          type: 'move',
          target: 'replacement',
  ...
  it('rejects stale, duplicate, unsupported, invalid-container and malformed-schema operations with typed failures', () => {
    const doc = documentWith([{ type: 'paragraph', attrs: { id: 'one' } }]);
    const expectCode = (input: any, code: string) =>
      expect(() => applyBlockOperations(doc, input)).toThrow(
        ...
        operations: [
          {
            type: 'insertAfter',
            target: 'one',
            content: { type: 'paragraph', attrs: { id: 'one' } },
          },
        ],
      },
      'DUPLICATE_BLOCK_ID',
    ...
    expectCode(
      {
        operations: [
          { type: 'insertAfter', target: 'one', content: { type: 'unknown' } },
        ],
      },
        ...
        operations: [
          {
            type: 'insertIn',
            target: 'one',
            content: { type: 'paragraph', attrs: { id: 'inside' } },
          },
        ],
      },
      'INVALID_OPERATION',
    ...
    expectCode(
      { operations: [{ type: 'update', target: 'one', attrs: { id: 2 } }] },
      'INVALID_BLOCK_SCHEMA',
    );
  ...
  it('promotes a legacy locator to an id and rejects missing targets without changing the document', () => {
    ...
    expect(
      (doc.getXmlFragment('default').get(0) as any).getAttribute('id'),
    ).toBe('promoted');
    expect(() =>
      applyBlockOperations(doc, {
        operations: [{ type: 'delete', target: 'missing' }],
      }),
    ).toThrow(expect.objectContaining({ code: 'BLOCK_NOT_FOUND' }));
  });
  ...
  it('preserves bold and link marks when moving formatted paragraph content', () => {
    ...
    applyBlockOperations(doc, {
      operations: [
        { type: 'move', target: 'one', destination: 'two', position: 'after' },
      ],
    });
    const moved: any = TiptapTransformer.fromYdoc(doc, 'default');
    expect(moved.content[1].content).toEqual(
  ...
  it('rejects update id collisions and inserting into leaf paragraph nodes', () => {
    ...
    expect(() =>
      applyBlockOperations(doc, {
        operations: [{ type: 'update', target: 'one', attrs: { id: 'two' } }],
      }),
        ...
        operations: [
          {
            type: 'insertIn',
            target: 'one',
            content: { type: 'paragraph', attrs: { id: 'three' } },
          },
        ],
      }),
        ...
        operations: [
          {
            ...
            content: {
              type: 'mention',
              attrs: { entityType: 'user', entityId: 'user-1' },
            },
          },
        ],
      }),
