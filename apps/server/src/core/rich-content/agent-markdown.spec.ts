...
describe('agent markdown', () => {
  it('round-trips nested columns and rich block attributes', async () => {
    ...
    expect(
      await agentMarkdownToProsemirror(markdown, tiptapExtensions),
    ).toMatchObject(doc);
  });
  ...
  it('keeps ordinary Markdown semantic after a round trip', async () => {
    ...
    expect(prosemirrorToAgentMarkdown(doc, tiptapExtensions)).toContain(
      '# Title',
    );
  });
  ...
  it.each([
    ...
    'keeps block directives literal inside code in %s',
  ...
  it.each([
    ...
    'keeps inline directives literal inside code in %s',
    async (_context, markdown) => {
      const doc = await agentMarkdownToProsemirror(markdown, tiptapExtensions);
  ...
  it.each([
    ...
    [
      '{{docmost:embed eyJpZCI6bnVsbCwiYXR0cnMiOnt9fQ}}',
    ...
    await expect(
      agentMarkdownToProsemirror(markdown, tiptapExtensions),
